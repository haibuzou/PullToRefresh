...
public class PullToRefreshListView extends
		PullToRefreshAdapterViewBase<ListView> implements OnClickListener{
	...
	public void failToLoad() {
		mFooterLoadingView.faileToLoad();
		isFailed = true;
//		dismissFooter();
	}

	public void noMoreData() {
		mFooterLoadingView.noMoreDara();
//		dismissFooter();
	}
	...
	public void showFooter() {
		...
		mFooterLoadingView.resetFoot();
	}

	
	@Override
	public void setMode(com.haibuzou.library.PullToRefreshBase.Mode mode) {
...
//		}
	...
	public void onRefreshing(boolean doScroll) {
		...
		handler.postDelayed(runnable, runOutTime);
	}
	...
	public ListView createListView(Context context, AttributeSet attrs,
		...
		String name = aa.getString(R.styleable.PullToRefresh_customListView);
		if (name != null) {
			int id = context.getResources().getIdentifier(name, "layout",
					context.getPackageName());
			if (id != 0) {
				ListView customListView = (ListView) inflate(context, id, null);
	...
	protected ListView createRefreshableView(Context context,
			AttributeSet attrs, TypedArray a) {
		ListView lv = createListView(context, attrs, a);
	...
	protected void handleStyledAttributes(TypedArray a) {
		...
		if (mListViewExtrasEnabled) {
			...
			mFooterLoadingView.setOnClickListener(this);
			mLvFooterLoadingFrame.addView(mFooterLoadingView, lp);
			mRefreshableView.addFooterView(mLvFooterLoadingFrame, null, false);

			// 原始的尾部直接去掉
			LoadingLayout oriFooter = getFooterLayout();
			oriFooter.hideAllViews();
			oriFooter.removeAllViews();
